...
const admin_ad_change_selling_rate_percent_scene = new BaseScene('admin_ad_change_selling_rate_percent_scene');
admin_ad_change_selling_rate_percent_scene.enter(async(ctx) => {
    ctx.session.ad = await $ad.findOne({ uid: ctx.scene.state.uid });

    await ctx.editMessageText(ctx.i18n.t("ad_change_selling_rate_percent_scene", {
...
admin_ad_change_selling_rate_percent_scene.on('text', async(ctx) => {
...
admin_ad_change_selling_rate_percent_scene.action('no', async(ctx) => {
    return ctx.scene.enter("admin_ad_change_selling_rate_percent_scene", { uid: ctx.session.ad.uid });
});

admin_ad_change_selling_rate_percent_scene.action('yes', async(ctx) => {
    const ad = await $ad.findOne({ uid: ctx.session.ad.uid });
    
    await ad.set("selling_rate_blockchain", true);
    await ad.set("selling_rate_blockchain_percent", ctx.session.percent);
